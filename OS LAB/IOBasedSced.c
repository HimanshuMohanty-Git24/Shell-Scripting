...
int main()
{
    ...
    {
        processes[i].pid = i + 1;
        printf("Enter the arrival time, burst time, and I/O time for process %d: ", processes[i].pid);
        scanf("%d%d%d", &processes[i].arrival_time, &processes[i].burst_time, &processes[i].io_time);
    ...
    while (completed_processes < n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                if (selected_process == -1 || processes[i].io_time < processes[selected_process].io_time)
                {
                    selected_process = i;
                }
            }
        ...
        processes[selected_process].io_time--;
        if (processes[selected_process].io_time == 0)
        {
            processes[selected_process].io_time = -1;
        }
        if (processes[selected_process].remaining_time == 0)
        {
            waiting_times[selected_process] = time - processes[selected_process].arrival_time - processes[selected_process].burst_time;
